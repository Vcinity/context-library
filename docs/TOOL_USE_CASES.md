...
1. **Implement within the current guidance.** Treat explicit decisions as
   constraints. Do not turn an inference or assumption into a stronger
   directive merely because it seems useful.
...
safety error; review the local edit rather than overwriting it.
